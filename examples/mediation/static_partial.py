"""Static partial-mediation example: x affects y directly and through m."""
...
def generate_static_partial_mediation(seed: int = 42, n: int = 450) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    x = rng.normal(size=n)
    m = 0.70 * x + rng.normal(scale=0.65, size=n)
    y = 0.45 * x + 0.65 * m + rng.normal(scale=0.65, size=n)
...
def build_config() -> CausalPipeConfig:
    return CausalPipeConfig(
        ...
        study_name="mediation_static_partial",
